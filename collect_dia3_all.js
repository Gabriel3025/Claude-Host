...
const PRODUTOS = [
  ...
  { rowIdx: 73, produto: "Jogo da Memória da Copa", link: "https://www.facebook.com/ads/library/?active_status=active&ad_type=all&id=1329141256012536&is_targeted_country=false&media_type=all&search_type=page&sort_data[mode]=total_impressions&sort_data[direction]=desc&view_all_page_id=1041598825693538" },
...
async function collectData() {
  ...
  try {
    ...
    console.log(`\n🚀 Iniciando coleta de ${PRODUTOS.length} produtos para DIA 3...\n`);

    for (let i = 0; i < PRODUTOS.length; i++) {
      const item = PRODUTOS[i];
      const percentual = Math.round(((i + 1) / PRODUTOS.length) * 100);

      try {
        console.log(`[${i + 1}/${PRODUTOS.length}] ${percentual}% - ${item.produto}...`);
        ...
        if (valor !== null) {
          ...
          falhas.push({ rowIdx: item.rowIdx, produto: item.produto, erro: 'Regex não encontrou valor' });
          console.log(`   ⚠️  Falha ao extrair`);
        }
      } catch (err) {
        falhas.push({ rowIdx: item.rowIdx, produto: item.produto, erro: err.message });
    ...
    console.log(`\n✅ Coleta concluída!`);
    console.log(`   Sucesso: ${resultados.length}/${PRODUTOS.length}`);
    console.log(`   Falhas: ${falhas.length}`);

    fs.writeFileSync('scratch_resultado_dia3.json', JSON.stringify({ resultados, falhas }, null, 2));
    console.log(`\n📝 Resultados salvos em scratch_resultado_dia3.json`);
