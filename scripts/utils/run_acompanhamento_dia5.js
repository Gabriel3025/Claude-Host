...
async function main() {
  console.log('📊 ACOMPANHAMENTO DE OFERTAS - DIA 5\n');

  try {
    // Passo 1: Gerar dados com read_sheet.js
    ...
    if (jsonStartLine === -1 || jsonEndLine === -1) {
      throw new Error('Não conseguiu encontrar JSON nos dados');
    }
    ...
    // Filtrar produtos de DIA 5
    const pendentes = allData.filter(p => p.precisaPreenchimento === true && p.diaNome === 'DIA 5');
    ...
    try {
      ...
      console.log('\n📁 Resultados salvos em: resultados_dia5.json');
      console.log('='.repeat(60) + '\n');
      console.log('📝 PRÓXIMO PASSO: node write_dia5_correto.js\n');
