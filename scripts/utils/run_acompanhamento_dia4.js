...
async function main() {
  console.log('📊 ACOMPANHAMENTO DE OFERTAS - DIA 4\n');
  ...
  try {
    ...
    const pendentes = allData.filter(p => p.precisaPreenchimento === true && p.diaNome === 'DIA 4');
    ...
    try {
      ...
      fs.writeFileSync(path.join(__dirname, 'resultados_dia4.json'), JSON.stringify(resultados, null, 2));

      console.log('\n📁 Resultados salvos em: resultados_dia4.json');
      console.log('='.repeat(60) + '\n');
