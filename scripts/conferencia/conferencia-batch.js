...
(async () => {
  ...
  for (let i = 0; i < produtos.length; i++) {
    ...
    try {
      await page.goto(prod.link, { waitUntil: 'networkidle2', timeout: 15000 });
      ...
      console.log(`[${i + 1}/${produtos.length}] ${prod.produto}: ${count}`);
    } catch (err) {
      results.push({
        idx: i + 1,
        rowIdx: prod.rowIdx,
      ...
      console.log(`[${i + 1}/${produtos.length}] ${prod.produto}: ERRO`);
    }
