...
// Todos os 48 produtos pendentes para DIA 3
const products = [
...
(async () => {
  ...
  console.log(`\n🔍 COLETA FINAL - ${products.length} PRODUTOS\n`);
    ...
    try {
      ...
      if (count === null) {
        const isEmpty = await page.evaluate(() => {
          const ads = document.querySelectorAll('[role="article"], .ad-item');
          return ads.length === 0;
        });
        count = isEmpty ? 0 : 0;
      }
  ...
  fs.writeFileSync('dados_coletados.json', JSON.stringify(results, null, 2));
  console.log('Gravando em write_results.js...\n');
