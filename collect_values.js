...
(async () => {
  ...
  for (let i = 0; i < produtos.length; i++) {
    ...
    try {
      ...
      await page.goto(p.link, { waitUntil: 'networkidle', timeout: 15000 });
      await page.waitForTimeout(1500);

      // Press Escape para fechar popup
      await page.press('Escape');
      await page.waitForTimeout(500);

      // Procurar pelo texto "~X resultados"
      const text = await page.innerText('body');
      const match = text.match(/~(\d+)\s*resultados/i);
      const valor = match ? match[1] : '0';
  ...
  fs.writeFileSync('collected_values.json', JSON.stringify(results, null, 2));
