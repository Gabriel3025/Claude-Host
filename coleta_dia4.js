...
const produtos = [
  ...
  { rowIdx: 92, produto: "Atividades para idosos", link: "https://www.facebook.com/ads/library/?active_status=active&ad_type=all&country=ALL&is_targeted_country=false&media_type=all&search_type=page&sort_data[mode]=total_impressions&sort_data[direction]=desc&view_all_page_id=118549594571851" },
...
(async () => {
  ...
  for (let i = 0; i < produtos.length; i++) {
    ...
    try {
      ...
      if (matchResultados) {
        ...
        falhas.push({ rowIdx: p.rowIdx, produto: p.produto, texto: texto.substring(0, 300) });
        console.log(`  -> FALHA na extração`);
      }
    } catch (err) {
      falhas.push({ rowIdx: p.rowIdx, produto: p.produto, erro: err.message });
  ...
  console.log('\n=== RESULTADOS ===');
  console.log(JSON.stringify({ resultados, falhas }, null, 2));
  console.log(`\nTotal coletados: ${resultados.length}/${produtos.length}`);
