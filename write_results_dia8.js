...
function getColunaDIA(dia) {
  // DIA 1 = G (6), DIA 2 = H (7), ..., DIA 8 = N (13)
  const colMap = {
...
async function main() {
  try {
    // Ler results
    const results = JSON.parse(fs.readFileSync('results_dia8.json', 'utf8'));
    ...
    const data = results.map(r => ({
      range: `${getColunaDIA(8)}${r.rowIdx + 1}`,
      values: [[r.valor]]
    }));
    ...
    console.log('📋 Resumo:');
    results.forEach(r => {
      console.log(`  [${getColunaDIA(8)}${r.rowIdx + 1}] ${r.produto}: ${r.valor} anúncios`);
    });

    console.log(`\n✅ Total: ${results.length} células atualizadas`);
  } catch (err) {
